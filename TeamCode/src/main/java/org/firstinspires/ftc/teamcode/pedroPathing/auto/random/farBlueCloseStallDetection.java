...
@Autonomous(name = "farBlueCloseStallDetection", group = "Autonomous")
@Configurable
@Disabled
public class farBlueCloseStallDetection extends OpMode {
    ...
    private int beltStallCount = 0;
    private double beltStallStart = 0;
    ...
    private void runBelt() {
        double powerToUse = (beltStallCount >= 2) ? 0.25 : 0.5;
        belt.setPower(powerToUse);

        boolean isSlow = belt.getVelocity() < 50;

        if (isSlow) {
            if (beltStallStart == 0) {
                beltStallStart = beltTimer.seconds();
            }
        } else {
            beltStallStart = 0;
            beltTimer.reset();
        ...
        if (beltStallStart != 0 && beltTimer.seconds() - beltStallStart > 0.5) {
            beltStallCount++;
            beltStallStart = 0;
            beltTimer.reset();
        }

        panelsTelemetry.debug("Belt Velocity", belt.getVelocity());
        panelsTelemetry.debug("Belt Stall Count", beltStallCount);
    }
    ...
    public static class Paths {
        ...
        public Paths(Follower follower) {
            Path1 = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(56.875, 8.563), new Pose(60.000, 75.500))
                    )
                    .setLinearHeadingInterpolation(Math.toRadians(90), Math.toRadians(306.5))
                    .build();
            ...
            Path3 = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(12.000, 84.000), new Pose(60.000, 75.500))
                    )
                    .setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(306.5))
                    .build();
            ...
            Path5 = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(9.000, 59.000), new Pose(60.000, 75.500))
                    )
                    .setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(306.5))
                    .build();
            ...
            Path6 = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(60.000, 75.500), new Pose(39.000, 15.000))
                    )
                    .setLinearHeadingInterpolation(Math.toRadians(306.5), Math.toRadians(270))
                    .build();
        }
    }
